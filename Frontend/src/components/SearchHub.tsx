...
import { GraduationCap, NotepadText, FileStack } from 'lucide-react';
...
export const SearchHub = ({ onSubmit, isLoading = false }: SearchHubProps) => {
  ...
  const handleAdditionalInfoSubmit = useCallback(() => {
    if (selectedCategory) {
      onSubmit(searchText, selectedCategory.styleNumber, additionalInfo.trim());
    }
    setShowAdditionalInfoDialog(false);
    setAdditionalInfo('');
  }, [onSubmit, searchText, additionalInfo, selectedCategory]);
