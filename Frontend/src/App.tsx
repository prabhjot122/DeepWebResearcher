...
const ChatDemo = () => {
  ...
  useEffect(() => {
    ...
    if (paths[1] === 'editor' && paths[2] === 'research' && paths[3]) {
      ...
      // Poll for research results if we have an active research
      if (researchId && !selectedItem) {
        const cleanup = pollResearchResults(researchId);
        return cleanup;
      }
    }
  ...
  }, [location.pathname, selectedItem, categoryFilter, libraryItems]);
